...
public interface UserNutritionLogsRepository extends JpaRepository<UserNutritionLogs, String> {
    ...
    @Query(value = """
            with body_Logs_latest as (
            	...
            	where ubl.measured_at::date <= :date and ubl.user_id =:userId
             ...
             select unl.user_id  ,
                       ...
                       where unl.user_id =: userId and unl.log_date::date = :date
                       group by unl.user_id,unl.log_date::date,ubl.weight_kg""", nativeQuery = true)
    UserNutritionGroupByUserAndLogDate getDailySummary(@Param("userId") String userId, @Param("date") LocalDate date);
    ...
    @Query(value = """
            select
                ...
                unl.log_date
            from project.user_nutrition_logs unl
            where unl.user_id =:userId
            order by unl.log_date
            """, nativeQuery = true)
    Page<UserNutritionPaginationResp> getLogsPagination(@Param("userId") String userId, Pageable pageable);
    ...
    @Query(value = """
            ...
            logs_data as (
                ...
                where unl.user_id =:userId       and
                (
                    (cast(:startDate as date) is null or cast(:endDate as date) is null)
                    or
                    (unl.log_date between :startDate and :endDate)
                )
            ...
            left join body_logs bl on bl.measured_at = ld.log_date
                                """, nativeQuery = true)
    Page<UserDailySummaryPagination> getSummaryPagination(@Param("userId") String userId, @Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate,
            Pageable pageable);
    ...
    @Query(value = """
             with log_data as (
            	...
            	user_id
            	from project.user_nutrition_logs unl
            	where unl.user_id =:userId and unl.log_date between :startOfWeek and :endOfWeek
            ...
            select
            	...
            	(bl.measured_at between :startOfWeek and :endOfWeek)
                                """, nativeQuery = true)
    UserWeeklySummary getUserSummary(@Param("userId") String userId,@Param("startOfWeek")  LocalDate startOfWeek,@Param("endOfWeek")  LocalDate endOfWeek);
